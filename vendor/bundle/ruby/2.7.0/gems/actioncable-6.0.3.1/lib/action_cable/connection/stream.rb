...
module ActionCable
  module Connection # Copyright (c) 2010-2015 James Coglan # # This class is heavily based on faye-websocket-ruby #--
    class Stream # :nodoc:
        ...
        if @write_lock.try_lock
          begin
            if @write_head.nil? && @write_buffer.empty?
              ...
              case written
              when :wait_writable # proceed below
              when data.bytesize
      ...
      def flush_write_buffer
        @write_lock.synchronize do
          loop do
            ...
            written =
              @rack_hijack_io.write_nonblock(@write_head, exception: false)
            case written
            when :wait_writable
              return false
      ...
      def hijack_rack_socket
        ...
        @rack_hijack_io = @socket_object.env['rack.hijack'].call # Retain existing behaviour if required:
