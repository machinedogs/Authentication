    ...
    class PostgreSQL < Base # :nodoc:
      ...
      def broadcast(channel, payload)
        with_broadcast_connection do |pg_conn|
          pg_conn.exec(
            "NOTIFY #{
              pg_conn.escape_identifier(channel_identifier(channel))
            }, '#{pg_conn.escape_string(payload)}'"
          )
        end
      end

      def subscribe(channel, callback, success_callback = nil)
        listener.add_subscriber(
      ...
      def with_subscriptions_connection(&block)
        # :nodoc:
        ar_conn =
          ActiveRecord::Base.connection_pool.checkout.tap do |conn|
      ...
      def listener
        @listener ||
          @server.mutex.synchronize do
            @listener ||= Listener.new(self, @server.event_loop)
          end
      end
          ...
          @adapter.with_subscriptions_connection do |pg_conn|
              ...
              loop do
                until @queue.empty?
                  ...
                  when :unlisten
                    pg_conn.exec(
                      "UNLISTEN #{pg_conn.escape_identifier channel}"
                    )
                  when :shutdown
                    throw :shutdown
                  end
