  ...
  module Channel
    ...
    class ConnectionStub
      ...
      def initialize(identifiers = {})
        ...
        @logger =
          ActiveSupport::TaggedLogging.new ActiveSupport::Logger.new(
                                             StringIO.new
                                           )
      end
      ...
      module Behavior
        ...
        def perform(action, data = {})
          ...
          subscription.perform_action(
            data.stringify_keys.merge('action' => action.to_s)
          )
        end
        ...
        def assert_no_streams
          assert subscription.streams.empty?,
                 "No streams started was expected, but #{
                   subscription.streams.count
                 } found"
        end
        ...
        def assert_has_stream(stream)
          assert subscription.streams.include?(stream),
                 "Stream #{stream} has not been started"
        end
        ...
        def check_subscribed!
          if subscription.nil? || subscription.rejected?
        ...
        end # Return only directly sent message (via #transmit)
      end
