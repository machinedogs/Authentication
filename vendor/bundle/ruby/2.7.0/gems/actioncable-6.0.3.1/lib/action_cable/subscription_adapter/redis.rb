      ...
      class Listener < SubscriberMap
        ...
        def listen(conn)
          conn.without_reconnect do
            original_client =
              conn.respond_to?(:_client) ? conn._client : conn.client
            ...
            conn.subscribe('_action_cable_internal') do |on|
              ...
              on.message { |chan, message| broadcast(chan, message) }

              on.unsubscribe do |chan, count|
                if count == 0
                  @subscription_lock.synchronize { @raw_client = nil }
                end
              end
            end
          end
        ...
        def when_connected(&block)
          @raw_client ? block.call : @when_connected << block
        end
