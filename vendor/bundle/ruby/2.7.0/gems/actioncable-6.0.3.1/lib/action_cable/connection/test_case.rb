...
module ActionCable
  module Connection
    ...
    module Assertions #   assert_reject_connection { connect params: { user_id: '' } } #   # Asserts that connection without user_id fails # # Asserts that the connection is rejected (via +reject_unauthorized_connection+).
      def assert_reject_connection(&block)
        assert_raises(
          Authorization::UnauthorizedError,
          'Expected to reject connection but no rejection was made',
          &block
        )
      end
    end
    ...
    class TestCase < ActiveSupport::TestCase
      module Behavior
        ...
        def connect(
          path = ActionCable.server.config.mount_path, **request_params
        )
          path ||= DEFAULT_PATH
          ...
          connection.send(
            :initialize,
            build_test_request(path, **request_params)
          )
        ...
        def build_test_request(
          path, params: nil, headers: {}, session: {}, env: {}
        )
          ...
          request_env =
            { 'QUERY_STRING' => query_string, 'PATH_INFO' => uri.path }.merge(
              env
            )

          if wrapped_headers.present?
            ActionDispatch::Http::Headers.from_hash(request_env).merge!(
              wrapped_headers
            )
          end
