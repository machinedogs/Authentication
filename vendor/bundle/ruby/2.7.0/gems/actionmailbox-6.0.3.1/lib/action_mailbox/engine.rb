...
module ActionMailbox
  class Engine < Rails::Engine
    ...
    config.action_mailbox.queues =
      ActiveSupport::InheritableOptions.new incineration:
    ...
    initializer 'action_mailbox.config' do
      config.after_initialize do |app|
        ...
        ActionMailbox.incinerate =
          if app.config.action_mailbox.incinerate.nil?
            true
          else
            app.config.action_mailbox.incinerate
          end
        ActionMailbox.incinerate_after =
          app.config.action_mailbox.incinerate_after || 30.days
