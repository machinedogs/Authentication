...
module ActionMailbox #    <tt>https://example.com/rails/action_mailbox/mailgun/inbound_emails/mime</tt>.
    ...
    class Authenticator
      ...
      def initialize(key:, timestamp:, token:, signature:)
        @key, @timestamp, @token, @signature =
          key, Integer(timestamp), token, signature
      end
      ...
      def signed?
        ActiveSupport::SecurityUtils.secure_compare signature,
                                                    expected_signature
      end
